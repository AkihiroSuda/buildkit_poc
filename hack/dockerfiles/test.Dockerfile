...
# available targets: buildd, buildd.oci_worker_only, buildd.containerd_worker_only
...
FROM buildkit-base AS buildd.oci_worker_only
ENV CGO_ENABLED=0
RUN go build -ldflags '-d'  -o /usr/bin/buildd.oci_worker_only -tags no_containerd_worker ./cmd/buildd

FROM buildkit-base AS buildd.containerd_worker_only
ENV CGO_ENABLED=0
RUN go build -ldflags '-d'  -o /usr/bin/buildd.containerd_worker_only -tags no_oci_worker ./cmd/buildd

FROM registry:$REGISTRY_VERSION AS registry

FROM unit-tests AS integration-tests
COPY --from=buildctl /usr/bin/buildctl /usr/bin/
COPY --from=buildd.containerd_worker_only /usr/bin/buildd.containerd_worker_only /usr/bin
COPY --from=buildd.oci_worker_only /usr/bin/buildd.oci_worker_only /usr/bin
...
FROM buildkit-export AS buildkit-buildd.oci_worker_only
COPY --from=buildd.oci_worker_only /usr/bin/buildd.oci_worker_only /usr/bin/
COPY --from=buildctl /usr/bin/buildctl /usr/bin/
ENTRYPOINT ["buildd.oci_worker_only"]

# Copy together all binaries for containerd worker mode
FROM buildkit-export AS buildkit-buildd.containerd_worker_only
COPY --from=runc /usr/bin/runc /usr/bin/
COPY --from=buildd.containerd_worker_only /usr/bin/buildd.containerd_worker_only /usr/bin/
COPY --from=buildctl /usr/bin/buildctl /usr/bin/
ENTRYPOINT ["buildd.containerd_worker_only"]
