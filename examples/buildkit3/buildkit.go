...
func buildkit(opt buildOpt) llb.State {
	...
	builddOCIWorkerOnly := run(llb.Shlex("go build -o /out/buildd.oci_worker_only -tags no_containerd_worker ./cmd/buildd"))

	builddContainerdWorkerOnly := run(llb.Shlex("go build -o /out/buildd.containerd_worker_only -tags no_oci_worker ./cmd/buildd"))
	...
	if opt.target == "containerd_worker_only" {
		return r.With(
			...
			copyAll(builddContainerdWorkerOnly, "/"))
	}
