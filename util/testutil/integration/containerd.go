...
func (c *containerd) New() (sb Sandbox, cl func() error, err error) {
	...
	if err := lookupBinary("buildd.containerd_worker_only"); err != nil {
		return nil, nil, err
	}
	...
	builddSock, stop, err := runBuildd([]string{"buildd.containerd_worker_only", "--containerd-worker-addr", address}, logs)
	if err != nil {
		return nil, nil, err
	}
	deferF.append(stop)
