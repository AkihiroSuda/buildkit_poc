...
func (ci *CacheImporter) ContentStore() content.Store {
	return ci.opt.ContentStore
}

func (ci *CacheImporter) Import(ctx context.Context, d digest.Digest) (instructioncache.InstructionCache, error) {
	return ci.ImportWithContentEnsurer(ctx, d, nil)
}

func (ci *CacheImporter) ImportWithContentEnsurer(ctx context.Context, d digest.Digest, ensurer func(context.Context, ocispec.Descriptor) error) (instructioncache.InstructionCache, error) {
	dt, err := content.ReadBlob(ctx, ci.opt.ContentStore, d)
	if err != nil {
		return nil, err
	}
	...
	if configDesc.Digest == "" {
		return nil, errors.Errorf("invalid build cache: %s", d)
	}

	if ensurer != nil {
		if err := ensurer(ctx, configDesc); err != nil {
			return nil, err
		}
	}
...
type importInfo struct {
	*CacheImporter
	...
	ensurer      func(context.Context, ocispec.Descriptor) error
}
		...
		for _, dp := range chain {
			func(dp blobs.DiffPair) {
				eg.Go(func() error {
					...
					if err := ii.ensurer(ctx, desc); err != nil {
						return err
					}
					return nil
				})
